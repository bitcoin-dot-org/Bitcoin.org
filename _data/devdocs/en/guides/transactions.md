...
They will extract the redeem script from the signature script, decode it, and execute it with the remaining stack items(the <sig> [sig..] part).
...
This last step is similar to the verification step in P2PKH or P2Multisig scripts, where the initial part of the signature script(<sig> [sig..]) acts as the "signature script" in P2PKH/P2Multisig, and the redeem script acts as the "pubkey script".
