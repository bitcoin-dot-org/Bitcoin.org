...
Bitcoin Core RPCs accept and return the byte-wise reverse of the computed
SHA-256 hash value. For example, the Unix `sha256sum` command displays the
...
The above result is also how the hash appears in the
...
However, since Bitcoin RPCs use the byte-wise reverse for hashes, if you
...
Bitcoin's use of hashes (which are byte arrays in C++) as integers
...
big-endian and little-endian, but they aren't always consistent. Worse,
these two different ways of representing a hash value can confuse anyone
who looks at the Bitcoin Core source code and finds a so-called "big-endian"
value being stored in a little-endian data type.
