  ...
  class EventPageGenerator < Generator
    def conferences
      ...
      YAML.load_file('_events.yml').each do |data|
        ...
        next if Time.new.to_i > (Time.new(date[0].to_i, date[1].to_i, date[2].to_i).to_i + 432_000)
        ...
        if data.key?('address')
          begin
            geoloc = JSON.parse(open(
              ...
              ) + '&sensor=false', 'User-Agent' => "Ruby/#{RUBY_VERSION}"
            ).read)
            if geoloc['status'] == 'OK'
              data['geoloc'] = {
                'lat' => geoloc['results'][0]['geometry']['location']['lat'].to_s,
                'lon' => geoloc['results'][0]['geometry']['location']['lng'].to_s
              }
            end
    ...
    def generate(site)
      ...
      unless site.respond_to?('conferences')
        class << site
          attr_accessor :meetups, :conferences

          alias_method :event_site_payload, :site_payload
          def site_payload
      ...
      if !ENV['ENABLED_PLUGINS'].nil? && ENV['ENABLED_PLUGINS'].index('events').nil?
      ...
      conferences_cache_unix_time = if File.exist?(conferences_cache)
                                      File.stat(conferences_cache).mtime.to_i
                                    else
                                      0
                                    end
